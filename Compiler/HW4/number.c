...
number_t number_parse(const char *text, NUMBER_TYPE yytype) {
    if (yytype == FLOAT) {
        return (number_t) {
            .type = FLOAT,
            .data.float_val = atof(text)
        };
    }
    return (number_t) {
        .type = INTEGER,
        .data.int_val = atoi(text)
    };
}
...
number_t number_plus(number_t n1, number_t n2) {
    ...
    if (n1.type == FLOAT) {
        if (n2.type == FLOAT)
            return (number_t) {
                .type = FLOAT,
                .data.float_val = n1.data.float_val + n2.data.float_val
            };
        ...
        return (number_t) {
            .type = FLOAT,
            .data.float_val = n1.data.float_val + n2.data.int_val
        };
    }
    if (n2.type == FLOAT) {
        ...
        return (number_t) {
            .type = FLOAT, 
            .data.float_val = n1.data.int_val + n2.data.float_val
        };
    }
    return (number_t) {
        .type = INTEGER,
        .data.int_val = n1.data.int_val + n2.data.int_val
    };
}
